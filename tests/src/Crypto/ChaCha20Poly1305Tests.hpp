...
TEST_CASE("Decrypt lorem_ipsum_ChaCha20Poly1305.txt.enigma") {
  const fs::path originalFilename = fs::path(TEST_DATA_DIR) / "lorem_ipsum.txt";
  const fs::path encryptedFilename = fs::path(TEST_DATA_DIR) / "lorem_ipsum_ChaCha20Poly1305.txt.enigma";
  const fs::path decryptedFilename = fs::temp_directory_path() / "lorem_ipsum_ChaCha20Poly1305.txt";
