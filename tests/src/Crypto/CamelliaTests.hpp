...
#include <Algorithm/Camellia/Camellia.hpp>
...
TEST_CASE("Camellia-GCM Encryption and Decryption - Text") {
  std::cout << "\n======[ " << Catch::getResultCapture().getCurrentTestName() << " ]======\n";

  // Make Camellia algorithm with intention to Encrypt and Decrypt
  std::unique_ptr<Camellia> camellia(new Camellia(Camellia::Intent::Encrypt | Camellia::Intent::Decrypt));
  ...
  std::vector<byte> encrypted = camellia->Encrypt(password, buffer);
  // Decrypted cipher (aka recovered)
  std::vector<byte> decrypted = camellia->Decrypt(password, encrypted);
...
TEST_CASE("Camellia-GCM Encryption and Decryption - File") {
  std::cout << "\n======[ " << Catch::getResultCapture().getCurrentTestName() << " ]======\n";
  ...
  camellia->Encrypt(password, filenameToEncrypt, encryptedFilename);
  camellia->Decrypt(password, encryptedFilename, recoveredFilename);
...
TEST_CASE("Decrypt lorem_ipsum_Camellia.txt.enigma") {
  const fs::path originalFilename = fs::path(TEST_DATA_DIR) / "lorem_ipsum.txt";
  const fs::path encryptedFilename = fs::path(TEST_DATA_DIR) / "lorem_ipsum_Camellia.txt.enigma";
  const fs::path decryptedFilename = fs::temp_directory_path() / "lorem_ipsum_Camellia.txt";
  FinalAction decryptedFileDeleter([decryptedFilename] {
    fs::remove(decryptedFilename);
  });

  try {
    Camellia camellia{Camellia::Intent::Decrypt};
    camellia.Decrypt("enigma@123", encryptedFilename, decryptedFilename);
