...
Camellia::Camellia(const Algorithm::Intent intent) noexcept
    : Algorithm(Algorithm::Type::Camellia, intent) {
  if (static_cast<bool>(intent & Intent::Encrypt))
    m_camellia_encryptor = std::make_unique<CryptoPP::GCM<CryptoPP::Camellia>::Encryption>();
  if (static_cast<bool>(intent & Intent::Decrypt))
    m_camellia_decryptor = std::make_unique<CryptoPP::GCM<CryptoPP::Camellia>::Decryption>();
}

Camellia::~Camellia() noexcept {
}

std::vector<byte> Camellia::Encrypt(const std::string& password, const byte *buffer, const std::size_t buffSize) {
  ENIGMA_ASSERT_OR_THROW(m_camellia_encryptor, "Camellia Encryptor is not initialized properly");
  ENIGMA_ASSERT_OR_THROW(password.size() >= Constants::Algorithm::MINIMUM_PASSWORD_LENGTH, "For security reasons, Camellia minimum password length is " + std::to_string(Constants::Algorithm::MINIMUM_PASSWORD_LENGTH));
  ...
  footer.iv = Algorithm::GenerateRandomIV(CryptoPP::Camellia::BLOCKSIZE);
  footer.hash = HashUtils::bytes<CryptoPP::SHA256>(buffer, buffSize);
  footer.extra = {};

  // Encryption key to be generated from user password + IV
  CryptoPP::SecByteBlock key(static_cast<std::size_t>(CryptoPP::Camellia::MAX_KEYLENGTH) + static_cast<std::size_t>(CryptoPP::Camellia::BLOCKSIZE));
  ...
  m_camellia_encryptor->SetKeyWithIV(key, static_cast<std::size_t>(CryptoPP::Camellia::MAX_KEYLENGTH), key + static_cast<std::size_t>(CryptoPP::Camellia::MAX_KEYLENGTH)); // key, kl, iv, ivl
...
std::vector<byte> Camellia::Encrypt(const std::string& password, const std::vector<byte>& buffer) {
  return Encrypt(password, buffer.data(), buffer.size());
}

std::vector<byte> Camellia::Decrypt(const std::string& password, const byte *cipher, const std::size_t cipherSize) {
  ENIGMA_ASSERT_OR_THROW(Meta::isEnigmaCipher(cipher, cipherSize), "Given cipher is malformed or was not encrypted with Enigma");
  ENIGMA_ASSERT_OR_THROW(m_camellia_decryptor, "Camellia Decryptor is not initialized properly");
  ...
  m_camellia_decryptor->SetKeyWithIV(key, static_cast<std::size_t>(CryptoPP::Camellia::MAX_KEYLENGTH), key + static_cast<std::size_t>(CryptoPP::Camellia::MAX_KEYLENGTH)); // key, kl, iv, ivl
...
std::vector<byte> Camellia::Decrypt(const std::string& password, const std::vector<byte>& cipher) {
  return Decrypt(password, cipher.data(), cipher.size());
}


void Camellia::Encrypt(const std::string& password, const fs::path& in_filename, const fs::path& out_filename) {
  ENIGMA_ASSERT_OR_THROW(!Meta::isEnigmaFile(in_filename), in_filename.filename().string() + " is already encrypted with Enigma");
  ENIGMA_ASSERT_OR_THROW(password.size() >= Constants::Algorithm::MINIMUM_PASSWORD_LENGTH, "Camellia Minimum Password Length is " + std::to_string(Constants::Algorithm::MINIMUM_PASSWORD_LENGTH));
  ENIGMA_ASSERT_OR_THROW(m_camellia_encryptor, "Camellia Encryptor is not initialized properly");
...
void Camellia::Decrypt(const std::string& password, const fs::path& in_filename, const fs::path& out_filename) {
