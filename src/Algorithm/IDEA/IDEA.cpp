...
std::vector<byte> IDEA::Encrypt(const std::string& password, const byte *buffer, const std::size_t buffSize) {
  ENIGMA_ASSERT_OR_THROW(m_idea_encryptor, "IDEA Encryptor is not initialized properly");
  ENIGMA_ASSERT_OR_THROW(password.size() >= Constants::Algorithm::MINIMUM_PASSWORD_LENGTH, "For security reasons, IDEA minimum password length is " + std::to_string(Constants::Algorithm::MINIMUM_PASSWORD_LENGTH));
  ...
  footer.iv = Algorithm::GenerateRandomIV(CryptoPP::IDEA::BLOCKSIZE);
  footer.hash = HashUtils::bytes<CryptoPP::SHA256>(buffer, buffSize);
  footer.extra = {};

  // Encryption key to be generated from user password + IV
  CryptoPP::SecByteBlock key(static_cast<std::size_t>(CryptoPP::IDEA::MAX_KEYLENGTH) + static_cast<std::size_t>(CryptoPP::IDEA::BLOCKSIZE));
  ...
  m_idea_encryptor->SetKeyWithIV(key, static_cast<std::size_t>(CryptoPP::IDEA::MAX_KEYLENGTH), key + static_cast<std::size_t>(CryptoPP::IDEA::MAX_KEYLENGTH)); // key, kl, iv, ivl
...
std::vector<byte> IDEA::Encrypt(const std::string& password, const std::vector<byte>& buffer) {
  return Encrypt(password, buffer.data(), buffer.size());
}

std::vector<byte> IDEA::Decrypt(const std::string& password, const byte *cipher, const std::size_t cipherSize) {
  ...
  m_idea_decryptor->SetKeyWithIV(key, static_cast<std::size_t>(CryptoPP::IDEA::MAX_KEYLENGTH), key + static_cast<std::size_t>(CryptoPP::IDEA::MAX_KEYLENGTH)); // key, kl, iv, ivl
...
std::vector<byte> IDEA::Decrypt(const std::string& password, const std::vector<byte>& cipher) {
  return Decrypt(password, cipher.data(), cipher.size());
}


void IDEA::Encrypt(const std::string& password, const fs::path& in_filename, const fs::path& out_filename) {
...
void IDEA::Decrypt(const std::string& password, const fs::path& in_filename, const fs::path& out_filename) {
