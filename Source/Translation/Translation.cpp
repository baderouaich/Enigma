		...
		if (std::ifstream ifs{ default_translation_file })
		{
			...
			while (!ifs.eof())
			{
				...
				if (std::getline(ifs, line); !line.empty())
				{
					...
					{
						m_english_texts.push_back(line);
					}
				}
			}
			ifs.close();
...
const String& Translation::Translate(const String& text)
{
	if (m_language == Language::English)
		return text; // english is by default
	if (m_translation.find(text) != m_translation.end())
...
void Translation::SetLanguage(const Language language)
{
	...
	m_translation.clear();
	for (const auto& text : m_english_texts)
	{
		m_translation[text] = m_config->Get<String>(StringifyLanguageEnum(m_language), text, text); // if translation not found, don't collapse! return english text instead.
	}
}
